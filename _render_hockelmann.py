...
W, H = 1600, 1000
variants = [
    ("hockelmann-a", "hockelmann_a.jpg"),
    ("hockelmann-b", "hockelmann_b.jpg"),
    ("hockelmann-c", "hockelmann_c.jpg"),
]
FORCE = """() => {
    const killSel = ['.overlay','.overlay-mask','.ov','.welcome','.modal','.pop','.sales','.sales-bar','#welcome','#welcomeMask','#greet','#offer','#pop','[data-mask]','[role=\"dialog\"]'];
    killSel.forEach(s=>document.querySelectorAll(s).forEach(e=>{e.classList.remove('show'); e.style.setProperty('display','none','important'); e.style.visibility='hidden'; e.style.opacity='0'; e.style.pointerEvents='none';}));
...
with sync_playwright() as p:
    ...
    for folder, outname in variants:
        ...
        page.screenshot(path=f"{OUT}/{outname}", type="jpeg", quality=86, clip={"x":0,"y":0,"width":W,"height":H})
